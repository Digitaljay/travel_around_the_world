...
def general(you_native_city,to_visit,date_to_go,budget):     # в поле to_visit должен быть массив городов, которые вы собираетесь посетить,
    ...
    def avia_cheapest(d_city,a_city,data,nextt):
        import requests
        import json
        import datetime
        data=data.split('.')
        ...
        for k in flies:
            if datetime.date(int(k['depart_date'].split('-')[0]),int(k['depart_date'].split('-')[1]),int(k['depart_date'].split('-')[2]))<=datetime.date(data[2],data[1],data[0])+datetime.timedelta(days=nextt):
                # newlist.append([int(k['value']))
                if k['depart_date'] not in newlist or int(k['value'])<newlist[k['depart_date']]:
                    newlist[k['depart_date']]=int(k['value'])
                dist=int(k['distance'])
    ...
    for i in range(len(order)-1):
        if order[i][1]=='самолёт':
            print('Не хотите ли рассмотреть другие даты для вылета из г.'+order[i][0]+'? (',avia_cheapest(order[i][0],order[i+1][0],date_to_go,20),')')
    return [coins,order,time_lost]
RESULT=general('Екатеринбург',['Казань','Санкт-Петербург'],'10.07.2019',60000)
